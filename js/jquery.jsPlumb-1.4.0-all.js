;(function() {
    ...
    jsPlumbUtil = {
        ...
        clone : function(a) {
            ...
            else if (this.isBoolean(a)) return !!a; //强制转换为布尔类型
        ...
        //
        // 查找函数链，找到第一个返回failvalue，没有则返回successsvalue
...
;(function() {
    ...
    var jsPlumbInstance = function(_defaults) {
        ...
        var log = null,
             ...
             * 绘制端点及其连接。这是绘制连接和端点的主要入口点，因为jsPlumb是在引擎罩下的端点中心。
		...
		*/
            ...
            _updateOffset = function(params) {
                ...
                if (recalc || !offset) { // if forced repaint or no offset available, we recalculate.
                    // get the current size and offset, and store them 获取当前大小和偏移量，并存储它们。
                    var s = _gel(elId);
                    if (s != null) {
    ...
    jsPlumb.AnchorManager = function(params) {
        ...
        this.redraw = function(elementId, ui, timestamp, offsetToUI, clearEdits) {
            if (!jsPlumbInstance.isSuspendDrawing()) {
                ...
                // 获取该元素所有端点 4.2
                var ep = _amEndpoints[elementId] || [], // 根据元素获取所有端点，div上的锚点
...
;                    // console.log("connectionsToPaint["+i+"].falg");
     ...
     * 一个浮锚。它的方向是从相对于锚的相对位置来动态计算的。在通过拖放创建连接时使用它。
 ...
 * 帮助类使用DOM元素的组件来使用未使用的鼠标事件，并被所有不同的呈现模式所使用。
 *
 */
;(function() {
...
// ************************** SVG utility methods ********************************************
    ...
    var SvgComponent = function(params) {
        ...
        this.getDisplayElements = function() {
            return displayElements; // 等同返回了svg组件
        };
...
(function () {
    ...
    var q = {
        ...
        return 0 == g ? Infinity : Math.atan(g /
            f)
    };
    window.jsBezier = {
        distanceFromCurve: x, gradientAtPoint: w, gradientAtPointAlongCurveFrom: function (a, b, f) {
            b = t(a, b, f);
            ...
            var f = x(a, b);
            return {point: v(b, b.length - 1, f.location, null, null), location: f.location}
        }, pointOnCurve: s, pointAlongCurveFrom: function (a, b, f) {
            return t(a, b, f).point
        }, perpendicularToCurveAt: function (a, b, f, d) {
            b = t(a, b, null == d ? 0 : d);
            a = w(a, b.location);
            d = Math.atan(-1 / a);
            a = f / 2 * Math.sin(d);
            f = f / 2 * Math.cos(d);
            return [{x: b.point.x + f, y: b.point.y + a}, {x: b.point.x - f, y: b.point.y - a}]
